...
if (Configure::read('debug')) {
    Configure::write('Cache._cake_model_.duration', env('CACHE_DURATION', '+2 minutes'));
    Configure::write('Cache._cake_core_.duration', env('CACHE_DURATION', '+2 minutes'));
 ...
 */
$pathLibs = dirname(__FILE__) . DS . 'libs';
foreach (scandir($pathLibs) as $filename) {
    $path = $pathLibs. DS . $filename;
    if (is_file($path)) {
        require $path;
    }
}
